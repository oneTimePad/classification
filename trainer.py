...
with tf.name_scope("train"):
    ...
    batcher = helpers.get_inputs(train_input_config, classification_model.preprocess)
...
if train_config.eval_while_training:
    with tf.name_scope("test"):
        classification_model_test = model_builder.build(model_config,
                                                         is_training = False,
                                                         reuse = True)
        batcher = helpers.get_inputs(eval_input_config, classification_model_test.preprocess)
        predictions = classification_model_test.predict(batched_tensors["input"])
        test_loss, test_scalar_updates = helpers.get_loss(predictions, batched_tensors)
        test_acc_dict = helpers.get_acc(predictions, batched_tensors)
